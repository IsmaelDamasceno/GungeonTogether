    ...
    public class SteamP2PManager
    {
        ...
        public void Initialise()
        {
            ...
            try
            {
                ...
                Debug.Log(
                    "Steam P2P Manager Initialised. Local SteamID will be retrieved when Steamworks is ready."
                );
                IsInitialised = true;
            }
            catch (System.TypeLoadException tle)
            {
                Debug.LogError(
                    $"SteamP2PManager: TypeLoadException - Type: {tle.TypeName}, Message: {tle.Message}"
                );
        ...
        private void TryRetrieveSteamId()
        {
            ...
            {
                Debug.LogWarning(
                    $"SteamP2PManager: Unexpected error retrieving Steam ID: {ex.GetType().Name}: {ex.Message}"
                );
            }
        }
        ...
        private void ReadPackets()
        {
            try
            {
                ...
                if (isAvailableMethod == null || readMethod == null)
                    return;
