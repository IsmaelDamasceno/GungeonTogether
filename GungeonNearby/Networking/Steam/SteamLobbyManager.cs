...
namespace GungeonNearby.Networking.Steam
{
    public class SteamLobbyManager
    {
        ...
        public void Initialise()
        {
            ...
            {
                Debug.LogWarning(
                    "[SteamLobby] Steamworks not available (not running Steam build???) [launch though steam]"
                );
                return;
            }
        ...
        public void CreateLobby(int maxMembers = 4)
        {
            ...
            try
            {
                ...
                object lobbyType =
                    GetLobbyTypeEnum("k_ELobbyTypeFriendsOnly")
                    ?? GetLobbyTypeEnum("k_ELobbyTypePublic");
                SteamReflectionHelper.CreateLobbyMethod.Invoke(
                    null,
                    new object[] { lobbyType, maxMembers }
                );
        ...
        public void LeaveLobby()
        {
            ...
            try
            {
                if (SteamReflectionHelper.LeaveLobbyMethod != null)
                {
                    ...
                    SteamReflectionHelper.LeaveLobbyMethod.Invoke(
                        null,
                        new object[] { lobbySteamId }
                    );
                }
            }
            catch { }
        ...
        public void OpenInviteDialog()
        {
            ...
            try
            {
                if (_steamworksAssembly == null)
                    return;
                Type steamFriendsType = _steamworksAssembly.GetType(
        ...
        private void HookCallbacks()
        {
            try
            {
                ...
                Type richPresenceJoinRequestedType =
                    SteamReflectionHelper.GameJoinRequestedCallbackType;

                if (_steamworksAssembly == null)
                {
                    Debug.LogWarning(
                        "[SteamLobby] Steamworks assembly is null, cannot hook callbacks"
                    );
                    return;
                }
                ...
                if (lobbyCreatedType != null)
                {
                    ...
                    _lobbyCreatedCb = SteamCallbackRouter.CreateCallback(
                        _steamworksAssembly,
                        lobbyCreatedType,
                        OnLobbyCreated
                    );
                    Debug.Log(
                        "[SteamLobby] LobbyCreated callback created: "
                            + (_lobbyCreatedCb != null ? "SUCCESS" : "FAILED")
                    );
                }
                else
                {
                ...
                if (lobbyEnterType != null)
                {
                    ...
                    _lobbyEnterCb = SteamCallbackRouter.CreateCallback(
                        _steamworksAssembly,
                        lobbyEnterType,
                        OnLobbyEnter
                    );
                    Debug.Log(
                        "[SteamLobby] LobbyEnter callback created: "
                            + (_lobbyEnterCb != null ? "SUCCESS" : "FAILED")
                    );
                }
                else
                {
                ...
                if (richPresenceJoinRequestedType != null)
                {
                    ...
                    _richPresenceJoinRequestedCb = SteamCallbackRouter.CreateCallback(
                        _steamworksAssembly,
                        richPresenceJoinRequestedType,
                        OnRichPresenceJoinRequested
                    );
                    Debug.Log(
                        "[SteamLobby] RichPresenceJoinRequested callback created: "
                            + (_richPresenceJoinRequestedCb != null ? "SUCCESS" : "FAILED")
                    );
                }
                else
                {
                ...
                if (
                    _lobbyCreatedCb == null
                    && _lobbyEnterCb == null
                    && _richPresenceJoinRequestedCb == null
                )
                {
                    Debug.LogError(
                        "[SteamLobby] Could not hook ANY callbacks (Steamworks callback API mismatch?)"
                    );
                }
                else
                {
        ...
        private void OnLobbyCreated(object callbackData)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        object lobbySteamId = SteamReflectionHelper.CreateCSteamID(lobbyId);
                        SteamReflectionHelper.SetLobbyDataMethod.Invoke(
                            null,
                            new object[]
                            {
                                lobbySteamId,
                                "gt_host",
                                SteamReflectionHelper.GetLocalSteamId().ToString(),
                            }
                        );
                        SteamReflectionHelper.SetLobbyDataMethod.Invoke(
                            null,
                            new object[]
                            {
                                lobbySteamId,
                                "gt_proto",
                                NetworkManager.ProtocolVersion.ToString(),
                            }
                        );
                    ...
                    {
                        object lobbySteamId = SteamReflectionHelper.CreateCSteamID(lobbyId);
                        SteamReflectionHelper.SetLobbyJoinableMethod.Invoke(
                            null,
                            new object[] { lobbySteamId, true }
                        );
                    ...
                    if (SteamReflectionHelper.SetRichPresenceMethod != null)
                    {
                        ...
                        SteamReflectionHelper.SetRichPresenceMethod.Invoke(
                            null,
                            new object[] { "connect", lobbyId.ToString() }
                        );
                        Debug.Log(
                            "[SteamLobby] Rich presence set to connect=" + lobbyId.ToString()
                        );
                    }
                    else
                    {
                ...
                {
                    Debug.LogError(
                        "[SteamLobby] Error setting rich presence: " + richPresenceEx.Message
                    );
                }
        ...
        private void OnLobbyEnter(object callbackData)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        object lobbySteamId = SteamReflectionHelper.CreateCSteamID(lobbyId);
                        object ownerCSteamId = SteamReflectionHelper.GetLobbyOwnerMethod.Invoke(
                            null,
                            new object[] { lobbySteamId }
                        );
                ...
                {
                    NetworkManager.Instance.ConnectTo(ownerId);
                }
            }
            catch (Exception e)
            {
        ...
        {
            Debug.Log(
                "[SteamLobby] ========== OnRichPresenceJoinRequested CALLBACK FIRED =========="
            );
            try
            {
                Debug.Log(
                    "[SteamLobby] Callback data type: "
                        + (callbackData != null ? callbackData.GetType().Name : "NULL")
                );

                // GameRichPresenceJoinRequested_t typically has a string connect field named m_rgchConnect
                string connect = ReadStringField(
                    callbackData,
                    "m_rgchConnect",
                    "m_connect",
                    "m_rgchConnectString"
                ...
                Debug.Log(
                    "[SteamLobby] Connect string: "
                        + (string.IsNullOrEmpty(connect) ? "(empty or null)" : connect)
                );

                if (string.IsNullOrEmpty(connect))
                {
                    Debug.Log(
                        "[SteamLobby] Join requested (empty connect string). Accepting invite might not include lobby id"
                    );
                    return;
                }
                ...
                {
                    Debug.Log(
                        "[SteamLobby] Join requested via rich presence. Extracted lobby id: "
                            + lobbyId
                    );
                    JoinLobby(lobbyId);
                }
                ...
                {
                    Debug.Log(
                        "[SteamLobby] Join requested but couldn't parse lobby id from connect string: "
                            + connect
                    );
                }
            }
