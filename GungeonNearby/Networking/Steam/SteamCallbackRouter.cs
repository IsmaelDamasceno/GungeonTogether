...
namespace GungeonNearby.Networking.Steam
{
    internal static class SteamCallbackRouter
    {
        ...
        {
            Systems.Logging.Debug.Log(
                "[SteamCallback] Invoke called with handlerIndex=" + handlerIndex
            );
            if (handlerIndex < 0 || handlerIndex >= _handlers.Count)
            {
                Systems.Logging.Debug.LogWarning(
                    "[SteamCallback] Invalid handler index: "
                        + handlerIndex
                        + " (count="
                        + _handlers.Count
                        + ")"
            ...
            {
                Systems.Logging.Debug.LogWarning(
                    "[SteamCallback] Handler at index " + handlerIndex + " is NULL"
                );
                return;
            }
            Systems.Logging.Debug.Log("[SteamCallback] Executing handler " + handlerIndex);
            handler(callbackData);
        }

        public static object CreateCallback(
            Assembly steamworksAssembly,
            Type callbackStructType,
        ...
        )
        {
            ...
            {
                Systems.Logging.Debug.LogWarning(
                    "[SteamCallback] CreateCallback: Assembly is null"
                );
                return null;
            }
            if (callbackStructType == null)
            {
                Systems.Logging.Debug.LogWarning("[SteamCallback] CreateCallback: Type is null");
                return null;
            }
            if (handler == null)
            {
                Systems.Logging.Debug.LogWarning("[SteamCallback] CreateCallback: Handler is null");
                return null;
            }
            ...
            {
                Systems.Logging.Debug.LogWarning("[SteamCallback] Callback`1 type not found");
                return null;
            }
            ...
            MethodInfo[] methods = callbackClosedType.GetMethods(
                BindingFlags.Public | BindingFlags.Static
            );
            for (int i = 0; i < methods.Length; i++)
            {
                if (!string.Equals(methods[i].Name, "Create"))
                    continue;
            ...
            {
                Systems.Logging.Debug.LogWarning(
                    "[SteamCallback] Create method not found for " + callbackStructType.Name
                );
                return null;
            }
            ...
            Systems.Logging.Debug.Log(
                "[SteamCallback] Registered handler index "
                    + handlerIndex
                    + " for "
                    + callbackStructType.Name
            );
            ...
            il.Emit(
                OpCodes.Call,
                typeof(SteamCallbackRouter).GetMethod(
                    "Invoke",
                    BindingFlags.Public | BindingFlags.Static
                )
            );
            ...
            {
                Systems.Logging.Debug.Log(
                    "[SteamCallback] Successfully created callback for " + callbackStructType.Name
                );
            }
            else
            {
                Systems.Logging.Debug.LogWarning(
                    "[SteamCallback] Callback creation returned null for " + callbackStructType.Name
                );
            }
