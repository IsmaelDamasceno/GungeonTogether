# GungeonNearby Build and Deploy Script for BepInEx
param(
    [switch]$Release,
...
Write-Host "Building GungeonNearby ($Configuration)..." -ForegroundColor Green
...
# Build the project
dotnet build GungeonNearby.csproj --configuration $Configuration
...
if (Test-Path $PluginsPath) {
    ...
    # Copy the DLL
    $SourceDLL = "bin\$Configuration\net35\GungeonNearby.dll"
    if (Test-Path $SourceDLL) {
        Copy-Item $SourceDLL $PluginsPath -Force
        Write-Host "Deployed GungeonNearby.dll" -ForegroundColor Green
    } else {
